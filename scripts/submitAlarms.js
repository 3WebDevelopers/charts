									...
									if (alarmNameArray[1] in allowedPatterns && 
										...
										fs.readFile(path.join('./', market, "DBINFO", alarm, 'Fundamental.txt'),
											...
											if(err) {
												...
												if (logErrors){
													if (fundamentalData.name == ''){console.log("EMPTY FUNDAMENTAL DATA NAME:"+path.join('./', market, alarm));}
													if (fundamentalDataArray.length != 12){console.log("INCORRECT FUNDAMENTAL DATA FIELDS:"+path.join('./', market, alarm));}
													return;
												}
